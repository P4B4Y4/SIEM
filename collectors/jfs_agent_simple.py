...
# -*- coding: utf-8 -*-
"""
JFS SIEM Agent - Simple Version (No Persistence)
Just GUI + basic service, no complex persistence
"""
import sys
import os
import json
...
class JFSAgentService(win32serviceutil.ServiceFramework):
    ...
    def SvcDoRun(self):
        logger.info("SvcDoRun() called - Service starting")
        
        # Report service as running to Windows IMMEDIATELY
        try:
            self.ReportServiceStatus(win32service.SERVICE_RUNNING)
            logger.info("Service status reported as RUNNING to Windows")
        except Exception as e:
            logger.warning(f"Could not report status: {e}")
        
        logger.info("Service running and waiting for stop event")
        try:
            while self.is_alive:
                try:
...
if __name__ == '__main__':
    logger.info("="*70)
    logger.info(f"Started - argv: {sys.argv}")
    logger.info("="*70)
    
    if len(sys.argv) > 1:
        # Service command
        logger.info(f"Service command: {sys.argv[1]}")
