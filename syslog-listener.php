 ...
 * Run as: php syslog-listener.php
...
class FortinetSyslogListener {
    ...
    public function __construct() {
        $this->log_file = __DIR__ . '/logs/syslog-listener.log';
        $this->log("=== Fortinet Syslog Listener Started ===");
    ...
    public function start() {
        ...
        if (!socket_bind($this->socket, '0.0.0.0', $this->port)) {
            ...
            $this->log("  2. Port 514 is not in use (check: netstat -ano | findstr :514)");
            $this->log("  3. Firewall allows UDP port 514");
            socket_close($this->socket);
            throw new Exception("Failed to bind to port {$this->port}: {$error}");
        }
        
        $this->log("✓ Listening on 0.0.0.0:{$this->port}");
        $this->log("Waiting for Fortinet syslog messages...");
    ...
    private function storeEvent($parsed, $from_ip, $raw_message) {
        try {
            ...
            $agent_id = 'FortiGate-' . FORTINET_IP;
            
            // Ensure agent exists
            $this->ensureAgentExists($agent_id, 'FortiGate', FORTINET_IP);
            ...
            if (preg_match('/srcip=(\S+)/', $content, $m)) {
                $source_ip = $m[1];
            }
            if (preg_match('/dstip=(\S+)/', $content, $m)) {
                $dest_ip = $m[1];
            }
            if (preg_match('/srcport=(\d+)/', $content, $m)) {
                $source_port = $m[1];
            }
            if (preg_match('/dstport=(\d+)/', $content, $m)) {
                $dest_port = $m[1];
            }
...
// Start listener
try {
    $listener = new FortinetSyslogListener();
